...
public final class CountryTest {
    ...
    public void shouldThrowException_whenCodeIsNull() {
        new Country(UUID.randomUUID().toString(), LocalDateTime.now(), null, "France");
    }

    @Test(expected = Exception.class)
    public void shouldThrowException_whenNameIsNull() {
        new Country(UUID.randomUUID().toString(), LocalDateTime.now(), "FR", null);
    }

    @Test
    public void shouldAssignACode() {
        Country country = new Country(UUID.randomUUID().toString(), LocalDateTime.now(), "FR", "France");
    ...
    public void shouldBeComparedOnCode() {
        Country country1 = new Country(UUID.randomUUID().toString(), LocalDateTime.now(), "BE", "Belgique");
        Country country2 = new Country(UUID.randomUUID().toString(), LocalDateTime.now(), "FR", "France");
